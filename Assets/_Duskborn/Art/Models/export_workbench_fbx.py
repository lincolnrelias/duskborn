import os
from generate_workbench import build_workbench

def export_fbx():
    builder = build_workbench()
    
    # Vertices (scale by 100 to cm, matching Chest.fbx and Tree.fbx)
...
CreationTime: "2026-09-18 08:30:00:000"
...
Objects:  {{
	...
	Material: "Material::Mat__Stone", "" {{
		Version: 102
		ShadingModel: "lambert"
		MultiLayer: 0
		Properties60:  {{
			Property: "DiffuseColor", "ColorRGB", "",0.55,0.55,0.55
		}}
	}}
	GlobalSettings:  {{
		Version: 1000
...
Connections:  {{
	Connect: "OO", "Model::Workbench", "Model::Scene"
	Connect: "OO", "Material::Mat__Wood", "Model::Workbench"
	Connect: "OO", "Material::Mat__Iron", "Model::Workbench"
	Connect: "OO", "Material::Mat__Stone", "Model::Workbench"
}}
"""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    fbx_path = os.path.join(script_dir, "Workbench.fbx")
    with open(fbx_path, "w", encoding="utf-8") as f:
        f.write(fbx_content)
    print(f"Exported Workbench.fbx with {len(verts)} vertices and {len(mat_indices)} polygons.")
